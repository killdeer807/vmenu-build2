    ...
    public class OnlinePlayers
    {
            ...
            menu.OnItemSelect += (sender, item, index) =>
                {
                    ...
                    if (player != null)
                    {
                        ...
                        playerMenu.MenuSubtitle = $"~s~Player: ~y~{GetDisplayName(currentPlayer)}";
        ...
        public async Task UpdatePlayerlist()
        {
            ...
            void UpdateStuff()
            {
                ...
                foreach (var p in MainMenu.PlayersList.OrderBy(GetDisplayName))
                {
                    var pItem = new MenuItem($"{GetDisplayName(p)}", $"Click to view the options for this player. Server ID: {p.ServerId}. Local ID: {p.Handle}.")
                    {
